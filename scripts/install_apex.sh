...
conf_ptbr() {
	echo "Include pt-br language"
	echo "change folder to apex language pt-br"
	cd /u01/app/oracle/apex/builder/pt-br
	echo "Set Variable NLS_LANG to English"
	set NLS_LANG=American_America.AL32UTF8
	echo "Altern Session to APEX_190100"
	$SQLPLUS -S $SQLPLUS_ARGS ALTER SESSION SET CURRENT_SCHEMA = APEX_190100 < /dev/null
	echo "Load pt-br language to APEX"
	$SQLPLUS -S $SQLPLUS_ARGS @load_pt-br.sql < /dev/null
}

unzip_apex(){
	echo "Extracting Apex-19.1"
	rm -rf /u01/app/oracle/apex
	unzip /files/apex_19.1.zip -d /u01/app/oracle/
}
